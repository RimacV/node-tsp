...
def read_distances_from_string(matrix):
    dists = []
    for idx, line in enumerate(matrix):
        lineWithoutComma = ''
        if(idx != len(sys.argv)-1):
            lineWithoutComma = line[:len(line)-1]
        else:
            lineWithoutComma = line

        # Skip comments and first argument (path to script)
        if lineWithoutComma[0] == '#' or idx == 0:
            continue

        dists.append(map(int, map(str.strip, lineWithoutComma.split(','))))
    return dists
...
if __name__ == '__main__':
    arg = sys.argv[1]
    if arg.endswith('.csv'):
        dists = read_distances_from_file(arg)
    elif len(sys.argv) > 1:
        dists = read_distances_from_string(sys.argv)
